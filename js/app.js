...
// LIFE_STAT_LABELS / META_STAT_LABELS → js/app-character.js
...
// STAT_DESCRIPTIONS / STAT_UI → js/app-character.js
...
// Character rendering/persona logic → js/app-character.js
// ══════════════════════════════════════════
// TAG COLORS & DROPDOWN
// ══════════════════════════════════════════
...
// capStat → js/app-character.js
function dateStamp(ts=Date.now()){
  const d=new Date(ts);
  return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
}
function rewardDateKey(ts=Date.now()){return dkey(new Date(ts));}
// getCharacter / saveCharacterIfNeeded → js/app-character.js
// XP curve / level / reward keys / award helpers / character stat calculation → js/app-xp.js
// getTopStats / getStrongestWeakestStats / generateRole / generateSuggestion → js/app-character.js
// renStatGrid / renderRecentXp / renderCharacter / renCharacter → js/app-character.js
